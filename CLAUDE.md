# traffic-source-cli — Project Notes

`ts` — terminal CLI for the [traffic-source](../traffic-source) analytics platform. Single binary, ~50KB, used by Kai + 2 technical teammates.
...
bun run build        # bundles to dist/ts.mjs with shebang + version inject
bun test             # bun test
bun run typecheck    # tsc --noEmit (must pass before commit)
bun link             # symlinks dist/ts.mjs to ~/.bun/bin/ts for local use
```

## Two-repo workflow
- Server repo: `~/projects/saas/traffic-source/` (Next.js, Zeabur). Endpoints, auth middleware, schema, mint-token script.
- CLI repo: this one (`~/projects/saas/traffic-source-cli/`). Distribution is `bun link` — not on npm.
...
- New endpoint or scope change in server → after deploy: re-test affected `ts` command end-to-end against `https://tritonix.cn`.

## Directory map
- `src/main.ts` — entrypoint. Awaits `dispatch(argv)`, exits with returned code.
- `src/dispatch.ts` — trie router. Walks argv tokens, longest-prefix match against `COMMANDS[]`. Intermediate nodes (`ts auth` with no verb) print children, never error.
...
- `src/config.ts` — `~/.ts/credentials.json` (token + api) and `~/.ts/config.json`. `resolveAuth(flags)` precedence: flag → `TS_API` env → file → default.
...
- `build.ts` — single file. Bun.build to `dist/ts.mjs`, prepends shebang, chmod 755, defines `process.env.CLI_VERSION` from package.json. **Bun-only APIs** (`import.meta.dir`, `Bun.build`) — that's why `@types/bun` is in tsconfig types.
...
- Spinners, progress, warnings, success ticks, command summaries → `stderr` (so `ts site ls | jq` still works).
...
## Auth & tokens
- Tokens are minted server-side (`scripts/mint-token.ts` in the server repo). The CLI never creates them.
- `ts auth login --token ts_…` saves to `~/.ts/credentials.json` (mode 0600). Token is sanity-checked against `/api/sites` on login — invalid tokens fail fast.
- Three scopes: `read` (queries), `write` (mutations like archive), `send` (outbound email). Server enforces; CLI doesn't pre-filter.
- `TS_API` env overrides API URL. There is no `TS_TOKEN` env — credentials are file-only or `--token` flag, by design (avoid ambient secrets in shells).
...
## Fetch timeouts
- `apiCall` always sets `AbortSignal.timeout(timeoutMs)`. Default 30s, override with `TS_TIMEOUT_MS` env. Streaming endpoints (none in v1) would skip.
...
- **`bun link` vs `npm link`**: both create global symlinks but in different prefixes. If `which ts` shows a path under `.nvm/versions/node/.../bin/`, an old `npm link` is winning over our `bun link`. Run `npm unlink -g @traffic-source/cli` to clean up.
