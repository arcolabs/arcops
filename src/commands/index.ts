...
export const COMMANDS: CommandDef[] = [
  ...
  { path: ['inbox', 'search'], summary: 'Search thread subjects across all sites (subject match)',
    positional: ['term'],
    flags: ['--status', { name: '--unread', type: 'bool' }, { name: '--limit', type: 'number' }, '--output'],
    examples: ['inbox search legnext', 'inbox search pricing --unread --limit 20'],
    handler: (a) => inbox.search(a) },
  { path: ['inbox', 'show'], summary: 'Show thread + messages (does not mark read; use `inbox read`)',
    positional: ['site', { name: 'thread-id', type: 'number' }], flags: [{ name: '--full', type: 'bool' }, '--output'],
  ...
  { path: ['inbox', 'reply'], summary: 'Queue reply (send scope; preview + typed-confirm unless --yes)',
    positional: ['site', { name: 'thread-id', type: 'number' }],
    flags: ['--body', '--body-file', '--template', { name: '--attach', type: 'string[]' }, { name: '--quote', type: 'bool' }, { name: '--reply-all', type: 'bool' }, { name: '--yes', type: 'bool' }, { name: '--dry-run', type: 'bool' }, '--idempotency-key', '--output'],
    examples: ['inbox reply acme.com 123 --body "Thanks for reaching out." --yes'],
    handler: (a) => inbox.reply(a) },
  { path: ['inbox', 'send'], summary: 'Queue a new email - creates a fresh thread (send scope)',
    positional: ['site'],
    flags: ['--to', '--cc', '--subject', '--from', '--body', '--body-file', '--template', { name: '--attach', type: 'string[]' }, { name: '--yes', type: 'bool' }, { name: '--dry-run', type: 'bool' }, '--output', '--idempotency-key'],
    examples: ['inbox send acme.com --to jane@example.com --subject "Hello" --body "Hi Jane" --yes'],
    handler: (a) => inbox.send(a) },
  { path: ['inbox', 'delivery', 'show'], summary: 'Show durable outbound delivery state',
    positional: ['site', 'delivery-id'], flags: ['--output'],
    examples: ['inbox delivery show acme.com idl_123'],
    handler: (a) => inbox.delivery.show(a) },
  { path: ['inbox', 'delivery', 'retry'], summary: 'Retry a failed delivery or explicitly retry an uncertain delivery (send scope)',
    positional: ['site', 'delivery-id'], flags: [{ name: '--confirm-duplicate-risk', type: 'bool' }, { name: '--yes', type: 'bool' }, '--output'],
    examples: ['inbox delivery retry acme.com idl_123 --yes'],
    handler: (a) => inbox.delivery.retry(a) },
  ...
  { path: ['inbox', 'draft', 'send'], summary: 'Queue a draft as an outbound reply (send scope)',
  ...
  // ── PLG GTM growth loop ──
  { path: ['growth', 'identity', 'upsert'], summary: 'Upsert product user/account identity and trusted Stripe link',
    positional: ['site'],
    flags: ['--user-id', '--email', '--user-traits', '--account-id', '--account-name', '--plan', '--role', '--account-traits', '--visitor-id', '--session-id', '--stripe-customer-id', '--idempotency-key', '--output'],
    examples: ['growth identity upsert acme.com --user-id usr_123 --account-id acct_456 --email founder@example.com'],
    handler: (a) => growth.identity.upsert(a) },
  { path: ['growth', 'lifecycle', 'show'], summary: 'Show active and historical PLG lifecycle definitions',
    positional: ['site'], flags: ['--output'],
    examples: ['growth lifecycle show acme.com'],
    handler: (a) => growth.lifecycle.show(a) },
  { path: ['growth', 'lifecycle', 'set'], summary: 'Activate a new immutable lifecycle definition',
    positional: ['site'], flags: ['--config', '--idempotency-key', '--output'],
    examples: ['growth lifecycle set acme.com --config @lifecycle.json'],
    handler: (a) => growth.lifecycle.set(a) },
  { path: ['growth', 'outcomes'], summary: 'Show acquisition → activation → paid → retained outcomes',
    positional: ['site'],
    flags: ['--from', '--to', { name: '--days', type: 'number' }, '--output'],
    examples: ['growth outcomes acme.com --days 30'],
    handler: (a) => growth.outcomes(a) },
  { path: ['growth', 'experiment', 'ls'], summary: 'List PLG growth experiments',
    positional: ['site'], flags: ['--output'],
    examples: ['growth experiment ls acme.com'],
    handler: (a) => growth.experiment.ls(a) },
  { path: ['growth', 'experiment', 'show'], summary: 'Show an experiment with releases and readbacks',
    positional: ['site', 'uid'], flags: ['--output'],
    examples: ['growth experiment show acme.com gx_abc123'],
    handler: (a) => growth.experiment.show(a) },
  { path: ['growth', 'experiment', 'create'], summary: 'Register an experiment measurement contract',
    positional: ['site'],
    flags: ['--name', '--hypothesis', '--target', '--method', '--metric', '--guardrails', '--baseline-from', '--baseline-to', '--observation-from', '--observation-to', '--status', '--idempotency-key', '--output'],
    ...
    handler: (a) => growth.experiment.create(a) },
  { path: ['growth', 'experiment', 'update'], summary: 'Update a draft contract or transition experiment status',
    positional: ['site', 'uid'],
    flags: ['--name', '--hypothesis', '--target', '--guardrails', '--status', '--idempotency-key', '--output'],
    examples: ['growth experiment update acme.com gx_abc123 --status running'],
    handler: (a) => growth.experiment.update(a) },
  { path: ['growth', 'experiment', 'readback'], summary: 'Measure an experiment and record an explicit decision',
    positional: ['site', 'uid'],
    flags: ['--from', '--to', { name: '--days', type: 'number' }, '--decision', '--notes', '--idempotency-key', '--output'],
    examples: ['growth experiment readback acme.com gx_abc123 --days 14 --decision keep --notes "Activation improved"'],
    handler: (a) => growth.experiment.readback(a) },
  { path: ['growth', 'release', 'ls'], summary: 'List releases in the growth ledger',
    positional: ['site'], flags: ['--experiment-uid', '--output'],
    examples: ['growth release ls acme.com --experiment-uid gx_abc123'],
    handler: (a) => growth.release.ls(a) },
  { path: ['growth', 'release', 'create'], summary: 'Record a deployed release',
    positional: ['site'],
    flags: ['--label', '--experiment-uid', '--version', '--change-url', '--deployed-at', '--metadata', '--idempotency-key', '--output'],
    examples: ['growth release create acme.com --label "Remove welcome step" --experiment-uid gx_abc123 --version 1.4.0'],
    handler: (a) => growth.release.create(a) },
  { path: ['growth', 'signal', 'ls'], summary: 'List deterministic PLG opportunity and risk signals',
    positional: ['site'], flags: ['--status', '--type', '--output'],
    examples: ['growth signal ls acme.com --status open'],
    handler: (a) => growth.signal.ls(a) },
  { path: ['growth', 'signal', 'refresh'], summary: 'Refresh deterministic signals and state transitions',
    positional: ['site'], flags: ['--idempotency-key', '--output'],
    examples: ['growth signal refresh acme.com'],
    handler: (a) => growth.signal.refresh(a) },
  { path: ['growth', 'signal', 'ack'], summary: 'Acknowledge an open growth signal',
    positional: ['site', 'uid'], flags: ['--idempotency-key', '--output'],
    examples: ['growth signal ack acme.com gs_abc123'],
    handler: (a) => growth.signal.ack(a) },
  { path: ['growth', 'signal', 'resolve'], summary: 'Resolve a growth signal with a required note',
    positional: ['site', 'uid'], flags: ['--note', '--idempotency-key', '--output'],
    examples: ['growth signal resolve acme.com gs_abc123 --note "Fixed onboarding instrumentation"'],
    handler: (a) => growth.signal.resolve(a) },
  { path: ['growth', 'doctor'], summary: 'Diagnose PLG instrumentation, identity, revenue, and signal readiness',
    positional: ['site'], flags: ['--output'],
    examples: ['growth doctor acme.com --output json'],
    handler: (a) => growth.doctor(a) },
