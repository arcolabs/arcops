...
export const VERBS: VerbDef[] = [
  ...
  {
    id: 'inbox:search',
    name: 'Search inbox threads across all sites',
    summary: 'Search thread subjects across all sites (subject match)',
    scope: 'read',
    idempotent: true,
    // CLI-orchestrated fan-out (single-site server endpoint called per site),
    // so it is local to the CLI and never exposed as a server-backed MCP tool.
    local: true,
    args: [
      { name: 'term', type: 'string', required: true, positional: true, description: 'Subject search term.' },
      { name: 'status', type: 'string', description: 'Filter by status (open/snoozed/closed).' },
      { name: 'unread', type: 'boolean', description: 'Only unread threads.' },
      { name: 'limit', type: 'number', description: 'Max threads per site (default 10, max 50).' },
      { name: 'output', type: 'string', cliOnly: true, description: 'Output format: text or json.' },
    ],
  ...
  },
  {
    ...
    args: [
      ...
      { name: 'reply_all', cliName: 'reply-all', type: 'boolean', description: 'CC the other participants of the last inbound message (default: reply only to the sender - the preview shows the real recipient, never a reply-all lie).' },
    ...
    http: { method: 'POST', path: '/api/sites/:siteId/inbox/threads/:threadId/reply', body: ['body', 'replyAll', 'bodyHtml'] },
    outputShape: 'unknown',
  },
  {
    id: 'inbox:send',
  ...
  // ── Events & webhooks ────────────────────────────────────────────────
  {
    ...
    examples: ['webhook ls'],
    http: { method: 'GET', path: '/api/webhook-endpoints' },
    outputShape: 'unknown',
  },
  {
    id: 'webhook:create',
    ...
    description: 'Creates a generic HTTPS webhook endpoint. The plaintext whsec_ signing secret is returned exactly once - save it immediately (lost = rotate via webhook:update --rotate-secret).',
    scope: 'write',
    idempotent: false,
    args: [
      { name: 'name', type: 'string', required: true, description: 'Endpoint display name.' },
      { name: 'url', type: 'string', required: true, description: 'HTTPS delivery URL (no RFC1918/loopback/arcops hosts).' },
      { name: 'event', type: 'string[]', required: true, repeatable: true, description: 'Subscribed event pattern: exact type, family wildcard (inbox.*), or *. Repeat for several.' },
      { name: 'site_filter', cliName: 'site-filter', type: 'string', description: 'Comma-separated site ids to restrict delivery (default: all sites).' },
      { name: 'output', type: 'string', cliOnly: true, description: 'Output format: text or json.' },
    ],
    examples: ['webhook create --name my-agent --url https://agent.example.com/arcops --event "inbox.*"'],
    http: { method: 'POST', path: '/api/webhook-endpoints', body: ['name', 'url', 'enabled_events', 'site_filter'] },
    outputShape: 'unknown',
  },
  {
    id: 'webhook:update',
    name: 'Update webhook endpoint',
    summary: 'Update a webhook endpoint (name/url/events/status/secret rotation)',
    description: 'PATCH an endpoint. --status active re-enables (also clears the circuit-breaker counter after auto_disabled). --rotate-secret mints a new whsec_ shown once in the response.',
    scope: 'write',
    idempotent: false,
    args: [
      { name: 'endpoint', type: 'string', required: true, positional: true, description: 'Endpoint id (we_…).' },
      { name: 'name', type: 'string', description: 'New display name.' },
      { name: 'url', type: 'string', description: 'New HTTPS delivery URL.' },
      { name: 'event', type: 'string[]', repeatable: true, description: 'Replace subscribed event patterns. Repeat for several.' },
      { name: 'site_filter', cliName: 'site-filter', type: 'string', description: 'Comma-separated site ids; "all" clears the filter.' },
      { name: 'status', type: 'enum', enum: ['active', 'disabled'], description: 'Manual disable / re-enable (auto_disabled recovery path).' },
      { name: 'rotate_secret', cliName: 'rotate-secret', type: 'boolean', description: 'Mint a new signing secret (shown once).' },
      { name: 'output', type: 'string', cliOnly: true, description: 'Output format: text or json.' },
    ],
    examples: ['webhook update we_abc --status active', 'webhook update we_abc --rotate-secret'],
    http: { method: 'PATCH', path: '/api/webhook-endpoints/:endpoint', body: ['name', 'url', 'enabled_events', 'site_filter', 'status', 'rotate_secret'] },
    outputShape: 'unknown',
  },
  {
    id: 'webhook:rm',
    ...
    http: { method: 'DELETE', path: '/api/webhook-endpoints/:endpoint' },
    outputShape: 'unknown',
  },
  {
    id: 'webhook:test',
    name: 'Test webhook endpoint',
    summary: 'Fire a real ping event at an endpoint (exits non-zero on failure)',
    description: 'Creates a ping event and delivers it synchronously through the real delivery state machine (signature included). Non-succeeded outcome exits non-zero with last_http_status/last_error.',
    scope: 'write',
    idempotent: false,
    args: [
      { name: 'endpoint', type: 'string', required: true, positional: true, description: 'Endpoint id (we_…).' },
    ...
    http: { method: 'POST', path: '/api/webhook-endpoints/:endpoint/test' },
    outputShape: 'unknown',
  },
  {
    id: 'webhook:deliveries',
    name: 'Endpoint delivery log',
    summary: 'Per-endpoint delivery log (cursor-paginated via --cursor)',
    scope: 'read',
    idempotent: true,
    args: [
      { name: 'endpoint', type: 'string', required: true, positional: true, description: 'Endpoint id (we_…).' },
      { name: 'status', type: 'enum', enum: ['pending', 'succeeded', 'failed', 'dead'], description: 'Filter by delivery status.' },
      { name: 'limit', type: 'number', description: 'Page size (max 200).' },
      { name: 'cursor', type: 'string', description: 'Opaque next-page cursor from a prior listing.' },
      { name: 'output', type: 'string', cliOnly: true, description: 'Output format: text or json.' },
    ],
    examples: ['webhook deliveries we_abc --status dead'],
    http: { method: 'GET', path: '/api/webhook-endpoints/:endpoint/deliveries', query: ['status', 'limit', 'cursor'] },
    outputShape: 'unknown',
  },
  {
    id: 'events:ls',
    name: 'List outbound events',
    summary: 'List outbound events (cursor-paginated via --cursor)',
    scope: 'read',
    idempotent: true,
    args: [
      { name: 'type', type: 'string', description: 'Filter by exact event type.' },
      { name: 'site', type: 'number', description: 'Filter by site id.' },
      { name: 'since', type: 'string', description: 'Only events at/after this ISO 8601 timestamp.' },
      { name: 'limit', type: 'number', description: 'Page size (max 200).' },
      { name: 'cursor', type: 'string', description: 'Opaque next-page cursor from a prior listing.' },
      { name: 'output', type: 'string', cliOnly: true, description: 'Output format: text or json.' },
    ],
    examples: ['events ls --type inbox.message.received --limit 20'],
    http: { method: 'GET', path: '/api/events', query: ['type', 'site', 'since', 'limit', 'cursor'] },
    outputShape: 'unknown',
  },
  {
    id: 'events:show',
    name: 'Show event',
    summary: 'Show one event with its deliveries',
    scope: 'read',
    idempotent: true,
    args: [
      { name: 'event', type: 'string', required: true, positional: true, description: 'Event id (evt_…).' },
      { name: 'output', type: 'string', cliOnly: true, description: 'Output format: text or json.' },
    ],
    examples: ['events show evt_abc'],
    http: { method: 'GET', path: '/api/events/:event' },
    outputShape: 'unknown',
  },
  {
    id: 'events:replay',
    name: 'Replay delivery',
    summary: 'Re-arm a failed/dead delivery (runner picks it up next tick)',
    scope: 'write',
    idempotent: true,
    args: [
      { name: 'delivery_id', cliName: 'delivery-id', type: 'number', required: true, positional: true, description: 'Delivery id (from events:show or webhook:deliveries).' },
      { name: 'output', type: 'string', cliOnly: true, description: 'Output format: text or json.' },
    ],
    examples: ['events replay 1234'],
    http: { method: 'POST', path: '/api/event-deliveries/:delivery_id/replay' },
    outputShape: 'void',
  },
